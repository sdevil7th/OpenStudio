...
bool JSFXProcessor::loadScript(const juce::String& path)
{
    ...
    if (!effect)
    {
        juce::Logger::writeToLog("JSFXProcessor: Failed to create YSFX instance");
        return false;
    }
    ...
    {
        juce::Logger::writeToLog("JSFXProcessor: Failed to load script: " + path);
        ysfx_free(effect);
        effect = nullptr;
        return false;
    }
    ...
    {
        juce::Logger::writeToLog("JSFXProcessor: Failed to compile script: " + path);
        ysfx_free(effect);
        effect = nullptr;
        return false;
    }
...
bool JSFXProcessor::reloadScript()
{
...
bool JSFXProcessor::isScriptLoaded() const
{
...
void JSFXProcessor::prepareToPlay(double sampleRate, int samplesPerBlock)
{
...
void JSFXProcessor::processBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midiMessages)
{
    if (!effect || !ysfx_is_compiled(effect))
        return;
...
void JSFXProcessor::releaseResources()
{
    // Nothing to release — YSFX handles its own memory
}

const juce::String JSFXProcessor::getName() const
{
    return effectName.isNotEmpty() ? effectName : "JSFX";
}

void JSFXProcessor::getStateInformation(juce::MemoryBlock& destData)
{
    if (!effect || !ysfx_is_compiled(effect))
        return;
    ...
    // Write magic + version
    stream.writeInt(0x4f534658); // "OSFX"
...
void JSFXProcessor::setStateInformation(const void* data, int sizeInBytes)
{
    juce::MemoryInputStream stream(data, static_cast<size_t>(sizeInBytes), false);

    int magic = stream.readInt();
    if (magic != 0x4f534658) // "OSFX"
        return;
...
bool JSFXProcessor::isBusesLayoutSupported(const BusesLayout& layouts) const
{
...
bool JSFXProcessor::hasGfxSection() const
{
    return effect != nullptr && ysfx_has_section(effect, ysfx_section_gfx);
}

bool JSFXProcessor::hasEditor() const
{
    return hasGfxSection();
}

juce::AudioProcessorEditor* JSFXProcessor::createEditor()
{
    if (hasGfxSection())
        return new JSFXGfxEditor(*this);
    return nullptr;
}

void JSFXProcessor::updateTimeInfo()
{
    if (!effect)
        return;

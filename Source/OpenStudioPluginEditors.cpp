...
OpenStudioLookAndFeel::OpenStudioLookAndFeel()
{
...
void OpenStudioLookAndFeel::drawRotarySlider(juce::Graphics& g, int x, int y, int w, int h,
...
void OpenStudioLookAndFeel::drawLinearSlider(juce::Graphics& g, int x, int y, int w, int h,
...
void OpenStudioLookAndFeel::drawToggleButton(juce::Graphics& g, juce::ToggleButton& btn,
...
void OpenStudioLookAndFeel::drawComboBox(juce::Graphics& g, int w, int h, bool isButtonDown,
...
void OpenStudioLookAndFeel::drawPopupMenuItem(juce::Graphics& g, const juce::Rectangle<int>& area,
...
juce::Font OpenStudioLookAndFeel::getComboBoxFont(juce::ComboBox&)
{
    return juce::Font(juce::FontOptions(13.0f));
}

juce::Font OpenStudioLookAndFeel::getPopupMenuFont()
{
    return juce::Font(juce::FontOptions(13.0f));
}
...
// ============================================================================
// OpenStudioLabeledKnob
// ============================================================================

OpenStudioLabeledKnob::OpenStudioLabeledKnob(const juce::String& name, const juce::String& sfx,
    ...
    : suffix(sfx)
{
    ...
    nameLabel.setColour(juce::Label::textColourId, juce::Colour(OpenStudioLookAndFeel::textDim));
    addAndMakeVisible(nameLabel);

    valueLabel.setJustificationType(juce::Justification::centred);
    valueLabel.setFont(juce::Font(juce::FontOptions(11.0f)));
    valueLabel.setColour(juce::Label::textColourId, juce::Colour(OpenStudioLookAndFeel::textPrimary));
...
void OpenStudioLabeledKnob::resized()
{
...
void OpenStudioLabeledKnob::setValue(float val, juce::NotificationType nt)
{
    slider.setValue(static_cast<double>(val), nt);
    updateValueLabel();
}

float OpenStudioLabeledKnob::getValue() const
{
    return static_cast<float>(slider.getValue());
}

void OpenStudioLabeledKnob::setSkew(float midPoint)
{
    slider.setSkewFactorFromMidPoint(static_cast<double>(midPoint));
}

void OpenStudioLabeledKnob::updateValueLabel()
{
    if (formatValue)
    {
...
OpenStudioModeToggle::OpenStudioModeToggle() {}

void OpenStudioModeToggle::setLabels(const juce::StringArray& labels)
{
    modeLabels = labels;
...
void OpenStudioModeToggle::paint(juce::Graphics& g)
{
    auto b = getLocalBounds().toFloat().reduced(1.0f);
    g.setColour(juce::Colour(OpenStudioLookAndFeel::bgSection));
    g.fillRoundedRectangle(b, 4.0f);
    g.setColour(juce::Colour(OpenStudioLookAndFeel::borderColor));
    ...
    for (int i = 0; i < numModes; ++i)
    {
        ...
        if (i == currentMode)
        {
            g.setColour(juce::Colour(OpenStudioLookAndFeel::accent).withAlpha(0.3f));
            g.fillRoundedRectangle(segBounds.reduced(2.0f), 3.0f);
        }

        g.setFont(11.0f);
        g.setColour(i == currentMode ? juce::Colours::white : juce::Colour(OpenStudioLookAndFeel::textDim));
        g.drawText(modeLabels[i], segBounds, juce::Justification::centred);
    }
}

void OpenStudioModeToggle::mouseDown(const juce::MouseEvent& e)
{
...
// ============================================================================
// OpenStudioGainReductionMeter
// ============================================================================

void OpenStudioGainReductionMeter::setGainReduction(float gr)
{
    grDB = gr;
    repaint();
}

void OpenStudioGainReductionMeter::paint(juce::Graphics& g)
{
    auto b = getLocalBounds().toFloat().reduced(1.0f);

    g.setColour(juce::Colour(OpenStudioLookAndFeel::bgKnob));
    ...
    if (barH > 0.5f)
    {
        auto grColor = grNorm > 0.7f ? juce::Colour(OpenStudioLookAndFeel::meterRed)
                     : grNorm > 0.4f ? juce::Colour(OpenStudioLookAndFeel::meterYellow)
                     : juce::Colour(OpenStudioLookAndFeel::accent);
        g.setColour(grColor);
    ...
    // dB labels
    g.setFont(9.0f);
    g.setColour(juce::Colour(OpenStudioLookAndFeel::textDim));
...
// ============================================================================
// OpenStudioSpectrumDisplay
// ============================================================================

OpenStudioSpectrumDisplay::OpenStudioSpectrumDisplay(OpenStudioEQ& eq) : eqProcessor(eq)
{
    startTimerHz(30);
}
...
void OpenStudioSpectrumDisplay::timerCallback()
{
    ...
    if (data.ready)
    {
        ...
        for (int i = 0; i < OpenStudioEQ::fftSize / 2; ++i)
        {
...
float OpenStudioSpectrumDisplay::freqToX(float freq, float w) const
{
...
float OpenStudioSpectrumDisplay::dbToY(float db, float h) const
{
...
void OpenStudioSpectrumDisplay::paint(juce::Graphics& g)
{
    ...
    const float binWidth = sampleRate / static_cast<float>(OpenStudioEQ::fftSize);

    auto drawSpectrum = [&](const float* data, juce::Colour color, float alpha)
    {
        juce::Path path;
        bool started = false;
        for (int i = 1; i < OpenStudioEQ::fftSize / 2; ++i)
        {
    ...
    drawSpectrum(smoothedPre, juce::Colour(OpenStudioLookAndFeel::textDim), 0.3f);
    drawSpectrum(smoothedPost, juce::Colour(OpenStudioLookAndFeel::accent), 0.8f);
    ...
    if (!magnitudes.empty())
    {
        ...
        g.setColour(juce::Colour(OpenStudioLookAndFeel::accentBright).withAlpha(0.6f));
...
// ============================================================================
// OpenStudioParametricEQGraph
// ============================================================================

OpenStudioParametricEQGraph::OpenStudioParametricEQGraph(OpenStudioEQ& eq) : eqProcessor(eq)
{
    startTimerHz(30);
...
float OpenStudioParametricEQGraph::freqToX(float freq) const
{
    return static_cast<float>(getWidth()) * (std::log10(freq / minFreq) / std::log10(maxFreq / minFreq));
}

float OpenStudioParametricEQGraph::xToFreq(float x) const
{
    float t = x / static_cast<float>(getWidth());
    return minFreq * std::pow(maxFreq / minFreq, t);
}

float OpenStudioParametricEQGraph::dbToY(float db) const
{
    float h = static_cast<float>(getHeight());
    return h * (1.0f - (db - minDB) / (maxDB - minDB));
}

float OpenStudioParametricEQGraph::yToDb(float y) const
{
    float h = static_cast<float>(getHeight());
    return minDB + (1.0f - y / h) * (maxDB - minDB);
}

int OpenStudioParametricEQGraph::findBandAt(float x, float y) const
{
    float closestDist = pointRadius * 2.0f;
    int closest = -1;
    for (int i = 0; i < OpenStudioEQ::numBands; ++i)
    {
...
int OpenStudioParametricEQGraph::findFirstDisabledBand() const
{
    for (int i = 0; i < OpenStudioEQ::numBands; ++i)
    {
        if (eqProcessor.bands[i].enabled.load() < 0.5f)
...
void OpenStudioParametricEQGraph::mouseDown(const juce::MouseEvent& e)
{
...
void OpenStudioParametricEQGraph::mouseDrag(const juce::MouseEvent& e)
{
...
void OpenStudioParametricEQGraph::mouseUp(const juce::MouseEvent&)
{
    dragging = false;
    dragBand = -1;
}

void OpenStudioParametricEQGraph::mouseMove(const juce::MouseEvent& e)
{
...
void OpenStudioParametricEQGraph::mouseWheelMove(const juce::MouseEvent& e, const juce::MouseWheelDetails& w)
{
...
void OpenStudioParametricEQGraph::mouseDoubleClick(const juce::MouseEvent& e)
{
    int band = findBandAt(e.position.x, e.position.y);
    if (band >= 0)
    {
...
void OpenStudioParametricEQGraph::paint(juce::Graphics& g)
{
    auto b = getLocalBounds().toFloat();
    const float w = b.getWidth();
    const float h = b.getHeight();
    ...
    if (hasSpectrumData)
    {
        ...
        drawSpec(smoothedPost, juce::Colour(OpenStudioLookAndFeel::accent), 0.15f);
    }
    ...
    if (!magnitudes.empty())
    {
        ...
        g.setColour(juce::Colour(OpenStudioLookAndFeel::accent).withAlpha(0.1f));
        g.fillPath(fillPath);
        g.setColour(juce::Colour(OpenStudioLookAndFeel::accentBright).withAlpha(0.8f));
        g.strokePath(curvePath, juce::PathStrokeType(2.0f));
    }
    ...
    // Instructions hint
    g.setFont(9.0f);
    g.setColour(juce::Colour(OpenStudioLookAndFeel::textDim).withAlpha(0.5f));
...
// ============================================================================
// OpenStudioInteractiveCompressorDisplay
// ============================================================================

OpenStudioInteractiveCompressorDisplay::OpenStudioInteractiveCompressorDisplay(OpenStudioCompressor& comp)
    : compressor(comp)
{
    startTimerHz(30);
}

void OpenStudioInteractiveCompressorDisplay::timerCallback()
{
...
float OpenStudioInteractiveCompressorDisplay::dbToX(float db) const
{
    return (db + dbRange) / dbRange * static_cast<float>(getWidth());
}

float OpenStudioInteractiveCompressorDisplay::dbToY(float db) const
{
    return static_cast<float>(getHeight()) - (db + dbRange) / dbRange * static_cast<float>(getHeight());
}

float OpenStudioInteractiveCompressorDisplay::xToDb(float x) const
{
    return x / static_cast<float>(getWidth()) * dbRange - dbRange;
}

float OpenStudioInteractiveCompressorDisplay::yToDb(float y) const
{
    return (1.0f - y / static_cast<float>(getHeight())) * dbRange - dbRange;
}

void OpenStudioInteractiveCompressorDisplay::mouseDown(const juce::MouseEvent& e)
{
...
void OpenStudioInteractiveCompressorDisplay::mouseDrag(const juce::MouseEvent& e)
{
...
void OpenStudioInteractiveCompressorDisplay::mouseUp(const juce::MouseEvent&)
{
    dragTarget = None;
}

void OpenStudioInteractiveCompressorDisplay::mouseMove(const juce::MouseEvent& e)
{
    float thresh = compressor.threshold.load();
    float threshX = dbToX(thresh);
    int newHovered = None;
...
void OpenStudioInteractiveCompressorDisplay::paint(juce::Graphics& g)
{
    auto b = getLocalBounds().toFloat();
    const float w = b.getWidth();
    const float h = b.getHeight();
    ...
    g.setColour(juce::Colour(OpenStudioLookAndFeel::accent));
    g.strokePath(curve, juce::PathStrokeType(2.0f));

    // Threshold marker (draggable)
    float threshX = dbToX(thresh);
    bool threshHovered = (hoveredTarget == Threshold || dragTarget == Threshold);
    g.setColour(juce::Colour(OpenStudioLookAndFeel::meterYellow).withAlpha(threshHovered ? 0.8f : 0.5f));
    g.drawVerticalLine(static_cast<int>(threshX), b.getY(), b.getBottom());

    // Threshold handle
    g.setColour(juce::Colour(OpenStudioLookAndFeel::meterYellow));
    ...
    if (displayInputLevel > -60.0f)
    {
        ...
        g.setColour(juce::Colour(OpenStudioLookAndFeel::meterGreen).withAlpha(0.4f));
        g.drawVerticalLine(static_cast<int>(inputX), b.getY(), b.getBottom());
    }

    // GR text
    g.setFont(14.0f);
    g.setColour(displayGR < -1.0f ? juce::Colour(OpenStudioLookAndFeel::meterYellow) : juce::Colour(OpenStudioLookAndFeel::textDim));
    ...
    // Hint
    g.setFont(9.0f);
    g.setColour(juce::Colour(OpenStudioLookAndFeel::textDim).withAlpha(0.4f));
...
// ============================================================================
// OpenStudioCompressorDisplay (legacy non-interactive)
// ============================================================================

OpenStudioCompressorDisplay::OpenStudioCompressorDisplay(OpenStudioCompressor& comp)
    : compressor(comp)
{
    startTimerHz(30);
}

void OpenStudioCompressorDisplay::timerCallback()
{
    displayGR = compressor.getCurrentGainReduction();
    displayInputLevel = compressor.getInputLevel();
    repaint();
...
void OpenStudioCompressorDisplay::paint(juce::Graphics& g)
{
    auto b = getLocalBounds().toFloat();
    const float w = b.getWidth();
    const float h = b.getHeight();
    ...
    g.setColour(juce::Colour(OpenStudioLookAndFeel::meterYellow).withAlpha(0.5f));
    g.drawVerticalLine(static_cast<int>(threshX), b.getY(), b.getBottom());
...
namespace
{
    ...
    void paintEditorBackground(juce::Graphics& g, juce::Component& comp, const juce::String& title)
    {
        ...
        // Gradient background
        g.setGradientFill(juce::ColourGradient(juce::Colour(OpenStudioLookAndFeel::bgDark), 0.0f, 0.0f,
                                                juce::Colour(OpenStudioLookAndFeel::bgPanel), 0.0f, b.getHeight(), false));
        g.fillAll();
...
OpenStudioEQEditor::OpenStudioEQEditor(OpenStudioEQ& p)
...
OpenStudioEQEditor::~OpenStudioEQEditor() { setLookAndFeel(nullptr); }

void OpenStudioEQEditor::setupBand(int idx)
{
    ...
    bc.freq = std::make_unique<OpenStudioLabeledKnob>("Freq", "Hz", 20.0f, 20000.0f, defaultFreqs[idx], 1.0f);
    bc.freq->setSkew(1000.0f);
    bc.freq->onValueChange = [this, idx](float v) { proc.bands[idx].freq = v; };
    addAndMakeVisible(*bc.freq);

    bc.gain = std::make_unique<OpenStudioLabeledKnob>("Gain", "dB", -30.0f, 30.0f, 0.0f, 0.1f);
    bc.gain->onValueChange = [this, idx](float v) { proc.bands[idx].gain = v; };
    addAndMakeVisible(*bc.gain);

    bc.q = std::make_unique<OpenStudioLabeledKnob>("Q", "", 0.1f, 30.0f, 1.0f, 0.01f);
    bc.q->setSkew(2.0f);
...
void OpenStudioEQEditor::syncFromProcessor()
{
    for (int i = 0; i < OpenStudioEQ::numBands; ++i)
    {
...
void OpenStudioEQEditor::timerCallback()
{
    // Sync values periodically (in case changed externally, e.g. automation)
    syncFromProcessor();
}

void OpenStudioEQEditor::paint(juce::Graphics& g)
{
    ...
    g.setColour(hpOn ? juce::Colour(OpenStudioLookAndFeel::accent) : juce::Colour(OpenStudioLookAndFeel::textDim));
    g.setFont(10.0f);
    ...
    g.setColour(lpOn ? juce::Colour(OpenStudioLookAndFeel::accent) : juce::Colour(OpenStudioLookAndFeel::textDim));
...
void OpenStudioEQEditor::resized()
{
    auto b = getLocalBounds();
...
// ============================================================================
// OpenStudioCompressorEditor
// ============================================================================

OpenStudioCompressorEditor::OpenStudioCompressorEditor(OpenStudioCompressor& p)
...
OpenStudioCompressorEditor::~OpenStudioCompressorEditor() { setLookAndFeel(nullptr); }

void OpenStudioCompressorEditor::syncFromProcessor()
{
...
void OpenStudioCompressorEditor::timerCallback() { syncFromProcessor(); }

void OpenStudioCompressorEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Compressor");
}

void OpenStudioCompressorEditor::resized()
{
    auto b = getLocalBounds();
    auto topBar = b.removeFromTop(headerH);
...
// ============================================================================
// OpenStudioGateEditor
// ============================================================================

OpenStudioGateEditor::OpenStudioGateEditor(OpenStudioGate& p)
...
OpenStudioGateEditor::~OpenStudioGateEditor() { setLookAndFeel(nullptr); }

void OpenStudioGateEditor::syncFromProcessor()
{
    thresholdKnob.setValue(proc.threshold.load());
...
void OpenStudioGateEditor::timerCallback()
{
    syncFromProcessor();
...
void OpenStudioGateEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Gate");

    // Gate indicator
    auto indBounds = gateIndicator.getBounds().toFloat();
    g.setColour(proc.isGateOpen() ? juce::Colour(OpenStudioLookAndFeel::meterGreen) : juce::Colour(OpenStudioLookAndFeel::meterRed));
    g.fillEllipse(indBounds.reduced(2.0f));
}

void OpenStudioGateEditor::resized()
{
    auto b = getLocalBounds();
    auto topBar = b.removeFromTop(headerH);
    modeToggle.setBounds(topBar.removeFromRight(140).reduced(4, 4));
...
// ============================================================================
// OpenStudioLimiterEditor
// ============================================================================

OpenStudioLimiterEditor::OpenStudioLimiterEditor(OpenStudioLimiter& p)
    : AudioProcessorEditor(p), proc(p),
...
OpenStudioLimiterEditor::~OpenStudioLimiterEditor() { setLookAndFeel(nullptr); }

void OpenStudioLimiterEditor::syncFromProcessor()
{
    thresholdKnob.setValue(proc.threshold.load());
    releaseKnob.setValue(proc.releaseMs.load());
...
void OpenStudioLimiterEditor::timerCallback()
{
    syncFromProcessor();
    grMeter.setGainReduction(proc.getGainReductionDB());
}

void OpenStudioLimiterEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Limiter");
}

void OpenStudioLimiterEditor::resized()
{
    auto b = getLocalBounds();
...
// ============================================================================
// OpenStudioDelayEditor
// ============================================================================

OpenStudioDelayEditor::OpenStudioDelayEditor(OpenStudioDelay& p)
    : AudioProcessorEditor(p), proc(p),
...
OpenStudioDelayEditor::~OpenStudioDelayEditor() { setLookAndFeel(nullptr); }

void OpenStudioDelayEditor::syncFromProcessor()
{
...
void OpenStudioDelayEditor::timerCallback() { syncFromProcessor(); }

void OpenStudioDelayEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Delay");
}

void OpenStudioDelayEditor::resized()
{
    auto b = getLocalBounds();
    auto topBar = b.removeFromTop(headerH);
    modeToggle.setBounds(topBar.removeFromRight(140).reduced(4, 4));
...
// ============================================================================
// OpenStudioReverbEditor
// ============================================================================

OpenStudioReverbEditor::OpenStudioReverbEditor(OpenStudioReverb& p)
    : AudioProcessorEditor(p), proc(p),
...
OpenStudioReverbEditor::~OpenStudioReverbEditor() { setLookAndFeel(nullptr); }

void OpenStudioReverbEditor::syncFromProcessor()
{
...
void OpenStudioReverbEditor::timerCallback() { syncFromProcessor(); }

void OpenStudioReverbEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Reverb");
}

void OpenStudioReverbEditor::resized()
{
    auto b = getLocalBounds();
    auto topBar = b.removeFromTop(headerH);
    modeToggle.setBounds(topBar.removeFromRight(140).reduced(4, 4));
...
// ============================================================================
// OpenStudioChorusEditor
// ============================================================================

OpenStudioChorusEditor::OpenStudioChorusEditor(OpenStudioChorus& p)
    : AudioProcessorEditor(p), proc(p),
...
OpenStudioChorusEditor::~OpenStudioChorusEditor() { setLookAndFeel(nullptr); }

void OpenStudioChorusEditor::syncFromProcessor()
{
...
void OpenStudioChorusEditor::timerCallback() { syncFromProcessor(); }

void OpenStudioChorusEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Chorus");
}

void OpenStudioChorusEditor::resized()
{
    auto b = getLocalBounds();
    auto topBar = b.removeFromTop(headerH);
    modeToggle.setBounds(topBar.removeFromRight(140).reduced(4, 4));
...
// ============================================================================
// OpenStudioSaturatorEditor
// ============================================================================

OpenStudioSaturatorEditor::OpenStudioSaturatorEditor(OpenStudioSaturator& p)
    : AudioProcessorEditor(p), proc(p),
...
OpenStudioSaturatorEditor::~OpenStudioSaturatorEditor() { setLookAndFeel(nullptr); }

void OpenStudioSaturatorEditor::syncFromProcessor()
{
...
void OpenStudioSaturatorEditor::timerCallback() { syncFromProcessor(); }

void OpenStudioSaturatorEditor::paint(juce::Graphics& g)
{
    paintEditorBackground(g, *this, "OpenStudio Saturator");
}

void OpenStudioSaturatorEditor::resized()
{
    auto b = getLocalBounds();
    auto topBar = b.removeFromTop(headerH);
    modeToggle.setBounds(topBar.removeFromRight(140).reduced(4, 4));

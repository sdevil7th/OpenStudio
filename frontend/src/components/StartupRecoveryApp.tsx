...
export function StartupRecoveryApp() {
  ...
  useEffect(() => {
    ...
    const invokeNativeFunction = async <T,>(name: string, ...args: unknown[]): Promise<T | undefined> => {
    ...
    const loadDiagnostics = async () => {
      try {
        const result = await invokeNativeFunction<StartupDiagnostics>("getStartupDiagnostics");
        if (!cancelled && result && typeof result === "object") {
          setDiagnostics(result);
        }
      } catch (error) {
        console.error("[StartupRecoveryApp] Failed to load startup diagnostics:", error);
      }
    };
  ...
  return (
    <div className="min-h-screen w-screen bg-daw-dark text-white flex flex-col">
      ...
      >
        <div className="px-3 text-xs text-neutral-400">OpenStudio - Safe Startup</div>
        <div
      ...
      <div className="flex-1 overflow-auto p-8">
        <div className="max-w-3xl mx-auto flex flex-col gap-6">
          <div className="space-y-3">
            <div className="text-2xl font-semibold text-neutral-100">Safe startup mode</div>
            <div className="text-sm text-neutral-400 leading-6">
              OpenStudio is running a minimal interface so you can verify that the embedded browser works,
              inspect startup diagnostics, and relaunch the full app after fixing any startup issue.
            </div>
          </div>

          <div className="rounded-xl border border-neutral-700 bg-neutral-900/80 p-5 space-y-4">
            <div className="text-sm font-semibold text-neutral-200">Startup diagnostics</div>
            <div className="grid gap-3 text-sm">
              <div><span className="text-neutral-500">Mode:</span> <span className="text-neutral-200">{diagnostics?.startupMode ?? "safe"}</span></div>
              <div><span className="text-neutral-500">Backend:</span> <span className="text-neutral-200">{diagnostics?.browserBackend ?? "unknown"}</span></div>
              <div><span className="text-neutral-500">State:</span> <span className="text-neutral-200">{diagnostics?.startupState ?? "loading"}</span></div>
              <div><span className="text-neutral-500">Startup log:</span> <span className="text-neutral-200 break-all">{diagnostics?.startupLogPath ?? "loading..."}</span></div>
              <div><span className="text-neutral-500">Frontend path:</span> <span className="text-neutral-200 break-all">{diagnostics?.packagedFrontendPath ?? "loading..."}</span></div>
              <div><span className="text-neutral-500">WebView2 runtime:</span> <span className="text-neutral-200">{diagnostics?.webView2RuntimeVersion ?? "n/a"}</span></div>
              {diagnostics?.targetUrl ? (
                <div><span className="text-neutral-500">Target URL:</span> <span className="text-neutral-200 break-all">{diagnostics.targetUrl}</span></div>
              ) : null}
            </div>
            {diagnostics?.detail ? (
              ...
              <div className="text-xs text-neutral-500">Loading more startup details...</div>
            )}
          </div>

          <div className="rounded-xl border border-neutral-700 bg-neutral-900/80 p-5 space-y-3">
            <div className="text-sm font-semibold text-neutral-200">Next steps</div>
            <div className="text-sm text-neutral-400 leading-6">
              Close this window and relaunch OpenStudio normally to retest the full interface. If startup still fails,
              collect the startup log above and run the installed-app inspection script from the repo.
            </div>

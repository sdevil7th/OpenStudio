...
function getDbPosition(db: number): number {
  return (1 - normalizeDbToMeter(db, "extended")) * 100;
}
...
export const ChannelStrip = React.memo(function ChannelStrip({
  ...
  const handleResetMeterClip = useCallback(() => {
    void nativeBridge.resetMeterClip(track.id);
  }, [track.id]);
  ...
  // Recomputes only when clips change; the tooltip value may lag by one render.
  const activeClipGainDB = useMemo(() => {
    if (isMaster) return null;
    const t = useDAWStore.getState().transport.currentTime;
    const clip = track.clips.find(
      (c) => t >= c.startTime && t < c.startTime + c.duration && !c.muted,
    );
  ...
  const trackGroup = trackGroups.find((g) =>
    g.memberTrackIds.includes(track.id),
  );
    ...
    (e: React.MouseEvent) => {
      ...
      if (trackGroup) {
        items.push({
          ...
          onClick: () => {
            const remaining = trackGroup.memberTrackIds.filter(
              (id) => id !== track.id,
            );
    ...
    <>
      ...
      >
        ...
        <div className="overflow-y-auto min-h-0">
          ...
          {isMaster && (
            <div className="flex gap-0.5 p-1 shrink-0">
              ...
              <Button
                ...
                aria-label={
                  masterMono
                    ? "Disable mono monitoring on master"
                    : "Enable mono monitoring on master"
                }
                className={classNames(
                  "flex-1",
                  masterMono
          ...
          <div
            className={classNames(
              "flex justify-between px-1 shrink-0",
              isMaster ? "pb-0.5" : "pb-1",
            )}
          >
        ...
        <div className="flex-1 flex gap-0.5 px-1 py-0.5 min-h-0 overflow-hidden">
          ...
          <div
            className={`relative ${CHANNEL_STRIP_DB_LABEL_WIDTH_CLASS} shrink-0 h-full`}
          >
        ...
        <div
          ...
          title={
            !isMaster
              ? `Clip: ${activeClipGainDB !== null ? formatGainDb(activeClipGainDB) : "--"}\nFader: ${formatGainDb(effectiveVolumeDB)}\nMaster: ${formatGainDb(masterVolumeDB)}\n────────\nSum: ${formatGainDb((activeClipGainDB ?? 0) + effectiveVolumeDB + masterVolumeDB)}`
              : undefined
          }
        >
        ...
        {!isMaster && (
          <div className="flex gap-0.5 p-1 shrink-0">
            <Button
              ...
              aria-label={
                track.muted
                  ? `Unmute track ${track.name}`
                  : `Mute track ${track.name}`
              }
              className="flex-1"
            >
              M
            ...
            <Button
              ...
              aria-label={
                track.armed
                  ? `Disarm recording on track ${track.name}`
                  : `Arm track ${track.name} for recording`
              }
              className="flex-1"
            >
              R

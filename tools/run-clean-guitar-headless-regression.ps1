param(
    [string]$AppPath,
    [string]$OutputRoot,
    [string]$Label = "clean-guitar-pitch-bend",
...
function New-RunDirectory {
    $root = if ($OutputRoot) { $OutputRoot } else { Join-Path $repoRoot "tmp_clean_guitar_runs" }
...
$resultPath = Join-Path $runDir "clean_guitar_regression_result.json"
$resolvedAppPath = Resolve-AppPath

$processStartInfo = [System.Diagnostics.ProcessStartInfo]::new()
$processStartInfo.FileName = $resolvedAppPath
$processStartInfo.Arguments = "--clean-guitar-regression-headless `"$resultPath`""
...
if (-not $process.WaitForExit($TimeoutSeconds * 1000)) {
    $process.Kill()
    throw "Clean guitar headless regression timed out after $TimeoutSeconds seconds."
}
if ($process.ExitCode -ne 0) {
    Write-Warning "Clean guitar headless regression exited with code $($process.ExitCode). Reading result if available."
}

if (-not (Test-Path $resultPath)) {
    throw "Clean guitar headless regression did not write result JSON: $resultPath"
}

$result = Get-Content -Raw -Path $resultPath | ConvertFrom-Json
Write-Host "Clean guitar regression result: $resultPath"

...
**Setup progress:** the panel above the model list stays visible while you scroll or select another tool. It shows the active model, current step and elapsed time. Hugging Face model transfers report bytes and a percentage; totals may increase as files are discovered and include resumed data. Dependency installation, conversion and validation show an active indicator when no byte total is available. Use **Show details** or **Open Install Log** for diagnostics. Closing the panel lets setup continue; keep OpenStudio running and reopen it with the **AI** button. **Cancel Setup** stops the job.

...
Release candidate notes for the current working tree. These changes are not in the published v0.1.01 installers. The committed source at [3beee3b](https://github.com/sdevil7th/OpenStudio/commit/3beee3b) has the same tree as v0.1.01; the changes described below are the uncommitted release preparation on top of that baseline. Commit and qualify this exact source before tagging or publishing.
...
- **Optional AI workflows:** add the Stable Audio Diffusers conversion/import path and MiniMax model import support, with request ownership and recovery independent of the dialog that started the job. AI execution still requires the appropriate managed runtime, model files and supported hardware.
